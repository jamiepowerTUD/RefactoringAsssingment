...
public class EmployeeDetails extends JFrame implements ActionListener, ItemListener, DocumentListener, WindowListener {
	...
	private JMenuBar menuBar() {
		...
		saveAs.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F2, InputEvent.CTRL_DOWN_MASK));
		...
		modify.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_DOWN_MASK));
		...
		closeApp.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F4, InputEvent.CTRL_DOWN_MASK));
	...
	private JPanel detailsPanel() {
		...
		for (int i = 0; i < empDetails.getComponentCount(); i++) {
			empDetails.getComponent(i).setFont(serif);
	...
	private void firstRecord() {
		...
		if (isSomeoneToDisplay()) {
			...
			currentEmployee = application.readRecords(0);
	...
	private Employee getChangedDetails() {
		...
		if (fullTimeCombo.getSelectedItem() != null && ((String) fullTimeCombo.getSelectedItem()).equalsIgnoreCase("Yes"))
			fullTime = true;
		...
		return new Employee(Integer.parseInt(idField.getText()), ppsField.getText().toUpperCase(),
				...
				Objects.requireNonNull(genderCombo.getSelectedItem()).toString().charAt(0), departmentCombo.getSelectedItem().toString(),
	...
	private String getFileName() {
		List<Character> chars =  "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_-".chars().mapToObj(e->(char)e).collect(Collectors.toList());
		Collections.shuffle(chars);
		return chars.stream().map(Object::toString).collect(Collectors.joining()).substring(0,19);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == closeApp && checkInput() && checkForChanges()) {
				...
				saveFile();
			change = false;
		} else if (e.getSource() == saveAs  && checkInput() && checkForChanges()) {
				saveFileAs();
			change = false;
